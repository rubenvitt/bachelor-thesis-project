...
Im Dezember 2018 suchte ich ein Thema, das ich für meine Bachelorarbeit verwenden konnte. Dazu habe ich viele Themen im [Katalog der möglichen Arbeiten für das Studium](https://pulse.fme.de/docs/DOC-3124) angeschaut und  mich letztendlich für die automatische Terminplanung entschieden.
...
Die Datenbankverbindung und -kommunikation funktionierte, mit den APIs konnte kommuniziert werden, triviale Anwendungsfunktionen waren realisiert (Liste aller Anwendungsnutzer ausgeben, Liste aller Räume, Liste aller Kalender eines Nutzers, …) - einzig die Terminplanung war offen. Bevor ich mich allerdings mit diesem Thema beschäftigen konnte, wollte ich mir einige Gedanken um die Systemsicherheit machen. Am liebsten hätte ich für meine Arbeit [Web-Authn](https://webauthn.guide) benutzt (funktioniert z.B. mit Chrome), doch dies wurde erst zu einem sehr fortgeschrittenen Zeitpunkt der Realisierung abschließend veröffentlicht. Um mich ein wenig auszuprobieren habe ich die Client-Middleware-Authentifizierung mit NodeJS komplett selbst realisiert. Dabei loggt sich ein Nutzer über das Frontend ein, diese Daten werden an die Middleware weitergeleitet. Der Nutzer wird nun im Backend authentifiziert (der Benutzer wird in der Datenbank gesucht und die Passwörter verglichen - ja, Klartextpasswörter sind eigentlich out...). War dies erfolgreich, so wird für den Nutzer ein Session-Token generiert und der Session angehängt, außerdem wird die UserID in einem Coockie gespeichert. Alle Anfragen werden im Anschluss authentifiziert, in dem die SessionID gesucht wird und überprüft, ob die AppuserID aus dem Coockie der SessionID zugeordnet ist. Ist dies nicht der Fall, wird der Benutzer zur Login-Seite weitergeleitet und die Anfrage abgebrochen.
...
Bei der Erstellung eines automatischen Termins gibt der Nutzer ein Startdatum und ein Enddatum eines Zeitraums an, in dem der Termin stattfinden soll. Außerdem gibt der Nutzer die Dauer des zu planenden Termins an. 20 Minuten oder auch 3 Stunden sind möglich, später könnten mehrtägige Termine implementiert werden.
Unter anderem diese Eigenschaften sowie alle Teilnehmer werden nun an das Backend gesendet, um einen Termin finden zu lassen. Das Backend empfängt dabei die Termine aller Terminteilnehmer von deren aktivierten Kalendern (bzw. von dessen APIs). Diese Termine werden in einer komplizierten Funktion zusammengefasst und mit
den, in den Einstellungen der Anwendung spezifizerten, Arbeitszeiten der Nutzer verglichen. Daraus wird letzten Endes eine Menge an Terminen generiert, an denen wirklich alle Teilnehmer (und der Veranstalter) Zeit haben sollten. Diese Terminvorschläge liegen zudem in der angegebenen Arbeitszeit aller Beteiligten.
Der Algorithmus dazu wird im UML-Diagramm weiter oben dargestellt, der Quellcode liegt im Gitlab.

Eine weitere angekündigte Funktion ist die automatische Raumsuche. In der Datenbank der Anwendung können Räume angelegt werden mit deren Ausstattungen. Also z.B. Raum Düsseldorf hat einen Fehrnseher, einen höhenverstellbaren Tisch, Fenster usw. Beim Anlegen eines Termins, egal ob automatisch oder manuell über die Anwendung
kann entweder ein fester Raum spezifizert werden, die Ausstattungen des Raums werden in der Anwendung nach der Auswahl aufgelistet. Oder es können Ausstattungen gewählt werden, zu denen ein Raum gefunden werden soll, der möglichst viele dieser Ausstattungen beinhaltet. In diesem Fall wird ein Raum ausgewählt, der zudem Platz
für alle Terminteilnehmer besitzt (auch in der Datenbank gepflegt).

In Zukunft könnte die Anwendung weiterentwickelt werden:
- automatische Termine könnten verschoben werden in einem gewissen Rahmen
- automatisch geplante Räume könnten geändert werden, falls diese von anderen Terminen benötigt werden
- Benachrichtigungen könnten implementiert werden
- Allgemein könnte die Anwendung zu einer Plattform werden, über die Termine geplant werden. Eine Anwendung, die in den Alltag jedes Mitarbeiters integriert wird.
- KI, die die Wünsche aller Nutzer analysiert und verbesserte Entscheidungen treffen kann. Zum Beispiel könnten damit Termine am Freitagnachmittag 15:30 verhindert werden, weil diese Vorschläge oft nicht akzeptiert werden, ...
